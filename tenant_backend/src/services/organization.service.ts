/**
 * ORGANIZATION SERVICE (SECURED WITH RLS WRAPPER)
 * * Tüm yetkilendirme auth_current_user_id() ile DB katmanında yapılır.
 * Servis sadece validasyon, tip kontrolü, veri dönüşümü ve 
 * RLS bağlamının (context) kurulmasından sorumludur.
 */
...
export const getUserOrganizations = async (userId: string): Promise<Organization[]> => {
    return withRLS(userId, async (client) => {
        try {
            const result = await client.query('SELECT * FROM get_user_organizations()');
...
export const getOrganizationById = async (orgId: string, userId: string): Promise<Organization | null> => {
    validateOrgId(orgId);
...
// ==================== DELETE ====================
export const deleteOrganization = async (orgId: string, userId: string): Promise<void> => {
    validateOrgId(orgId);
...
// ==================== MEMBERS ====================
export const getOrganizationMembers = async (orgId: string, userId: string): Promise<OrganizationMember[]> => {
    validateOrgId(orgId);
...
export const removeMember = async (orgId: string, memberId: string, userId: string): Promise<void> => {
    validateOrgId(orgId);
    if (!isValidUUID(memberId)) throw new AppError(ErrorCodes.VALIDATION_INVALID_UUID);

    return withRLS(userId, async (client) => {
        try {
            await client.query('SELECT remove_member($1, $2)', [orgId, memberId]);
...
// ==================== INVITATIONS ====================
export const getPendingInvitations = async (orgId: string, userId: string): Promise<OrganizationInvitation[]> => {
    validateOrgId(orgId);
    return withRLS(userId, async (client) => {
        try {
            const result = await client.query('SELECT * FROM get_pending_invitations($1)', [orgId]);
            return result.rows;
        } catch (error: any) {
            if (error.message.includes('PERMISSION_DENIED')) {
                throw new AppError(ErrorCodes.ORG_PERMISSION_DENIED,
...
export const getInvitationOrgId = async (invitationId: string, userId: string): Promise<string | null> => {
    return null;
};
...
// ==================== STATS ====================
export const getOrganizationStats = async (orgId: string, userId: string): Promise<OrganizationStats | null> => {
    validateOrgId(orgId);
    return withRLS(userId, async (client) => {
        try {
            const result = await client.query('SELECT * FROM get_organization_stats($1)', [orgId]);
...
// ==================== LEAVE ====================
export const leaveOrganization = async (orgId: string, userId: string): Promise<void> => {
    validateOrgId(orgId);

    return withRLS(userId, async (client) => {
        try {
            await client.query('SELECT leave_organization($1)', [orgId]);
