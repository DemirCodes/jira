...
exports.getUserTokenVersion = exports.revokeAllUserTokens = exports.isTokenBlacklisted = exports.blacklistToken = exports.authMiddleware = void 0;
 ...
 */
const checkTokenVersion = async (userId, tokenVersion) => {
    if (process.env.NODE_ENV === 'test') {
        return true;
    }
    try {
        const result = await tenantPool_1.tenantPool.query('SELECT token_version FROM users WHERE user_id = $1', [userId]);
        ...
        logger_1.log.error('Token version check failed', { userId: safeLogUserId(userId), error });
        return false;
    }
};
...
const authMiddleware = async (req, res, next) => {
    ...
    try {
        ...
        if (!ipRateLimit.allowed) {
            logger_1.log.warn('Rate limit exceeded (IP)', { ip: req.ip, path: req.path });
        ...
        if (!decoded.userId || !decoded.tokenVersion) {
            logger_1.log.warn('Auth failed: Missing required fields in token payload');
        ...
        if (!(0, regexValidator_1.isValidUUID)(decoded.userId)) {
            ...
            logger_1.log.warn('Auth failed: Invalid UUID format in token', { userId: safeUserId });
        ...
        if (tokenAge > maxTokenAge) {
            const safeUserId = safeLogUserId(decoded.userId);
            logger_1.log.warn('Auth failed: Token too old', { userId: safeUserId, age: tokenAge });
        ...
        if (!isValidVersion) {
            const safeUserId = safeLogUserId(decoded.userId);
            logger_1.log.warn('Auth failed: Token version mismatch - revoked', { userId: safeUserId });
        ...
        try {
            ...
            logger_1.log.error('Auth failed: Sanitization error', { error });
        ...
        try {
            ...
            await tenantPool_1.tenantPool.query('SELECT set_config($1, $2, false)', ['app.current_user_id', sanitizedUserId]);
            await tenantPool_1.tenantPool.query('SELECT set_config($1, $2, false)', ['app.current_token_iat', decoded.iat.toString()]);
            await tenantPool_1.tenantPool.query('SELECT set_config($1, $2, false)', ['app.current_token_version', decoded.tokenVersion.toString()]);
        }
        catch (error) {
            logger_1.log.error('Auth failed: RLS setup error', {
                error: error.message,
        ...
        if (isProduction) {
            ...
            logger_1.log.error('Auth unexpected error', { errorId, duration: `${duration}ms` });
...
const revokeAllUserTokens = async (userId) => {
    try {
        ...
        const result = await tenantPool_1.tenantPool.query('UPDATE users SET token_version = COALESCE(token_version, 0) + 1, updated_at = NOW() WHERE user_id = $1 RETURNING token_version', [userId]);
        const newVersion = result.rows[0]?.token_version;
        const safeUserId = safeLogUserId(userId);
        logger_1.log.info('All user tokens revoked', {
            userId: safeUserId,
        ...
        logger_1.log.error('Failed to revoke user tokens', { userId: safeLogUserId(userId), error });
...
const getUserTokenVersion = async (userId) => {
    const result = await tenantPool_1.tenantPool.query('SELECT COALESCE(token_version, 1) as token_version FROM users WHERE user_id = $1', [userId]);
    if (result.rows.length === 0) {
        throw new Error('User not found');
    }
    return result.rows[0].token_version;
};
exports.getUserTokenVersion = getUserTokenVersion;
