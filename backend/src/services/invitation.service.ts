...
export const acceptInvitation = async (invitationId: string): Promise<void> => {
    ...
    try {
        await tenantPool.query('SELECT accept_invitation($1)', [invitationId]);
...
export const rejectInvitation = async (invitationId: string): Promise<void> => {
    ...
    try {
        await tenantPool.query('SELECT reject_invitation($1)', [invitationId]);
...
// ==================== READ ====================
export const getPendingInvitationsForUser = async (): Promise<InvitationWithDetails[]> => {
    try {
        const result = await tenantPool.query(
            `SELECT i.*, o.org_name, 
...
export const getPendingInvitationsForOrg = async (orgId: string): Promise<InvitationWithDetails[]> => {
    if (!isValidUUID(orgId)) throw new AppError(ErrorCodes.VALIDATION_INVALID_UUID);
