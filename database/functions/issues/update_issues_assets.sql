-- update_issue_asset.sql

create or replace function update_issue_asset(
    p_asset_id uuid,
...
begin
    ...
    if not (auth_is_issue_contributor(v_issue_id) or v_uploaded_by = v_user_id) then
        raise exception 'Permission denied: Only issue contributor or uploader can update this asset';
    end if;
